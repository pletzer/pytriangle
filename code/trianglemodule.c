...
triangulate_SET_POINTS(PyObject *self, PyObject *args) {
  ...
  if(!PyCapsule_CheckExact(address)) {
    snprintf(MSG, MSG_SIZE, "ERROR in %s at line %d: wrong argument #1 (triangulateio handle required)\n", __FILE__, __LINE__);
    PyErr_SetString(PyExc_TypeError, MSG);
    return NULL;
  }    
  if(!PySequence_Check(xy)) {
    snprintf(MSG, MSG_SIZE, "ERROR in %s at line %d: wrong argument #2 ([(x, y), ...] required)\n", __FILE__, __LINE__);
    PyErr_SetString(PyExc_TypeError, MSG);
    return NULL;
  }
  if(!PySequence_Check(mrks)) {
    snprintf(MSG, MSG_SIZE, "ERROR in %s at line %d: wrong argument #3 ([m, ...] required)\n", __FILE__, __LINE__);
    PyErr_SetString(PyExc_TypeError, MSG);
    return NULL;
  }
  object = (struct triangulateio *) PyCapsule_GetPointer(address, TRIANGULATEIO_NAME);  
  ...
  if(npts > 0) {
    if(npts != object->numberofpoints) {
      ...
      /* if(object->pointattributelist) free(object->pointattributelist); */
      if(object->pointmarkerlist) free(object->pointmarkerlist);
      object->pointlist = (REAL *) malloc(_NDIM * npts * sizeof(REAL));
      /* object->pointattributelist = malloc(natts * npts * sizeof(REAL)); */
      object->pointmarkerlist = (int *) malloc(npts * sizeof(int));
    }
...
triangulate_SET_POINT_ATTRIBUTES(PyObject *self, PyObject *args) {
  ...
  if(!PySequence_Check(atts)) {
    snprintf(MSG, MSG_SIZE, "ERROR in %s at line %d: wrong argument #2 ([(a0, ...),...] required)\n", __FILE__, __LINE__);
    PyErr_SetString(PyExc_TypeError, MSG);
    return NULL;
  }
  object = (struct triangulateio *) PyCapsule_GetPointer(address, TRIANGULATEIO_NAME);  
  ...
  if(object->pointattributelist) free(object->pointattributelist);
  object->pointattributelist = (REAL *) malloc(natts * npts * sizeof(REAL));
...
triangulate_SET_TRIANGLE_ATTRIBUTES(PyObject *self, PyObject *args) {
  ...
  if(object->triangleattributelist) free(object->triangleattributelist);
  object->triangleattributelist = (REAL *) malloc(natts * ntri * sizeof(REAL));
...
triangulate_SET_SEGMENTS(PyObject *self, PyObject *args) {
  ...
  if(!PySequence_Check(segs)) {
    snprintf(MSG, MSG_SIZE, "ERROR in %s at line %d: wrong argument #2 ([(ia, ib),...] required)\n", __FILE__, __LINE__);
    PyErr_SetString(PyExc_TypeError, MSG);
    return NULL;
  }
  if(!PySequence_Check(marks)) {
    snprintf(MSG, MSG_SIZE, "ERROR in %s at line %d: wrong argument #3 ([m1,m2,...] required)\n", __FILE__, __LINE__);
    PyErr_SetString(PyExc_TypeError, MSG);
    return NULL;
  }
  object = (struct triangulateio *) PyCapsule_GetPointer(address, TRIANGULATEIO_NAME);  
  ...
  if(ns != object->numberofsegments) {
    ...
    object->segmentmarkerlist = (int *) malloc( ns * sizeof(int));
  }
...
triangulate_SET_REGIONS(PyObject *self, PyObject *args) {
  ...
  if(!PySequence_Check(xy)) {
    snprintf(MSG, MSG_SIZE, "ERROR in %s at line %d: wrong argument #2 ([(x, y, r , a),...] required)\n", __FILE__, __LINE__);
    PyErr_SetString(PyExc_TypeError, MSG);
    return NULL;
  }
  object = (struct triangulateio *) PyCapsule_GetPointer(address, TRIANGULATEIO_NAME);
...
triangulate_TRIANGULATE(PyObject *self, PyObject *args) {
  ...
  if(!PyCapsule_CheckExact(address_in)) {
    snprintf(MSG, MSG_SIZE, "ERROR in %s at line %d: wrong argument #2 (input triangulateio handle required)\n", __FILE__, __LINE__);
    PyErr_SetString(PyExc_TypeError, MSG);
    return NULL;
  }    
  if(!PyCapsule_CheckExact(address_out)) {
    snprintf(MSG, MSG_SIZE, "ERROR in %s at line %d: wrong argument #3 (output triangulateio handle required)\n", __FILE__, __LINE__);
    PyErr_SetString(PyExc_TypeError, MSG);
    return NULL;
  }    
  ...
  object_out->holelist = (REAL *) malloc(_NDIM * object_in->numberofholes * sizeof(REAL));
  ...
  if(object_in->numberofregions > 0) {
    ...
    object_out->regionlist = (REAL *) malloc(4 * object_in->numberofregions * sizeof(REAL));
...
triangulate_GET_NUM_POINTS(PyObject *self, PyObject *args) {
  ...
  if(!PyCapsule_CheckExact(address)) {
    snprintf(MSG, MSG_SIZE, "ERROR in %s at line %d: wrong argument (triangulateio handle required)\n", __FILE__, __LINE__);
    PyErr_SetString(PyExc_TypeError, MSG);
    return NULL;
  }
  object = (struct triangulateio *) PyCapsule_GetPointer(address, TRIANGULATEIO_NAME);
